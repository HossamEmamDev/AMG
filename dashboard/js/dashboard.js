...
// ── Auth ──
const SUPER_ADMIN = { username: 'admin', password: '987654321*', role: 'Super Admin' };

function getTeam() { return JSON.parse(localStorage.getItem('amg_team') || '[]'); }
function setTeam(t) { localStorage.setItem('amg_team', JSON.stringify(t)); }
...
function handleLogin(e) {
  ...
  if (user) {
    ...
    document.getElementById('current-user-label').textContent = user.role || 'Sub-Admin';
    initDashboard();
  } else {
...
function switchTab(tab) {
  ...
  document.getElementById('topbar-title').textContent = navEl?.querySelector('span')?.textContent?.replace(/\d+/, '').trim() || tab;
  document.getElementById('sidebar').classList.remove('open');
}
...
function loadSettings() {
  ...
  document.getElementById('s-primary-color').value   = s.primaryColor   || '#B8860B';
  document.getElementById('s-secondary-color').value = s.secondaryColor || '#2B2B2B';
  document.getElementById('s-bg-color').value         = s.bgColor        || '#FAFAF8';
  const nb = document.getElementById('s-nav-bg');   if(nb) nb.value = s.navBg   || '#FFFFFF';
  const nt = document.getElementById('s-nav-text'); if(nt) nt.value = s.navText || '#1A1A1A';
  document.getElementById('s-font-en').value = s.fontEn || 'Barlow';
  document.getElementById('s-font-ar').value = s.fontAr || 'Cairo';
  const fhe = document.getElementById('s-font-heading-en'); if(fhe) fhe.value = s.fontHeadingEn || 'Playfair Display';
...
function previewFont(type, name) {
  ...
  if (!document.getElementById(id) && name !== 'Barlow' && name !== 'Cairo') {
    const lk = document.createElement('link'); lk.id = id; lk.rel = 'stylesheet';
  ...
  if (type === 'en' || type === 'heading-en') box.style.fontFamily = `'${name}', serif`;
}
...
function saveSettings() {
  ...
  const nb2 = document.getElementById('s-nav-bg');   if(nb2) s.navBg   = nb2.value;
  const nt2 = document.getElementById('s-nav-text'); if(nt2) s.navText = nt2.value;
  s.fontEn          = document.getElementById('s-font-en').value;
  s.fontAr          = document.getElementById('s-font-ar').value;
  const fhe2 = document.getElementById('s-font-heading-en'); if(fhe2) s.fontHeadingEn = fhe2.value;
  ...
  document.documentElement.style.setProperty('--dash-accent', s.primaryColor || '#B8860B');
...
function _paintSocialLinks() {
  ...
  const iconOptions = [
    ...
    'fab fa-whatsapp','fab fa-twitter','fab fa-youtube',
    'fab fa-tiktok','fab fa-telegram','fab fa-snapchat'
  ];
  wrap.innerHTML = socialTemp.map((sl, i) => `
...
function renderSectionsManager() {
  ...
  if (!wrap) return;
  const names = { 'our-group':'Our Group','services':'Services','projects':'Projects','careers':'Careers','partners':'Partners','contact':'Contact' };
  wrap.innerHTML = Object.entries(sections).map(([id, cfg]) => `
...
function saveSections() { showMsg('sections-msg', 'Section settings saved!', true); }
...
function renderOverview() {
  ...
  if (recentMsgs) {
    if (!messages.length) { recentMsgs.innerHTML = '<div class="empty-state"><i class="fas fa-inbox"></i>No messages yet</div>'; }
    else recentMsgs.innerHTML = messages.slice(-4).reverse().map(m => `
  ...
  if (recentApps) {
    if (!apps.length) { recentApps.innerHTML = '<div class="empty-state"><i class="fas fa-user-tie"></i>No applications yet</div>'; }
    else recentApps.innerHTML = apps.slice(-4).reverse().map(a => `
      <div class="msg-row">
...
// ── Services ──
const ICONS = ['fas fa-building','fas fa-bolt','fas fa-road','fas fa-paint-roller','fas fa-chart-gantt','fas fa-industry','fas fa-hammer','fas fa-drafting-compass','fas fa-hard-hat','fas fa-wrench','fas fa-fire','fas fa-water','fas fa-wind','fas fa-solar-panel','fas fa-tools','fas fa-ruler-combined','fas fa-sitemap','fas fa-network-wired','fas fa-tower-broadcast','fas fa-bridge','fas fa-city','fas fa-warehouse','fas fa-home','fas fa-hotel','fas fa-mosque','fas fa-hospital','fas fa-school','fas fa-leaf','fas fa-recycle','fas fa-shield-alt'];

function renderServicesList() {
  const services = getData('services') || [];
  const list = document.getElementById('services-list');
  if (!list) return;
  if (!services.length) { list.innerHTML = '<div class="empty-state" style="grid-column:1/-1"><i class="fas fa-tools"></i>No services yet</div>'; return; }
...
function saveService(id) {
  ...
  if (id) { const idx = services.findIndex(x=>x.id===id); if(idx>-1) services[idx]=item; }
...
function renderProjectsList() {
  ...
  if (!list) return;
  if (!projects.length) { list.innerHTML = '<div class="empty-state" style="grid-column:1/-1"><i class="fas fa-building"></i>No projects yet</div>'; return; }
...
function renderProjectImgList() {
  ...
  if (!projectImagesTemp.length) {
    wrap.innerHTML = '<span style="font-size:0.75rem;color:var(--dash-muted);padding:4px">No images yet — add URLs or upload files</span>';
    return;
  }
  wrap.innerHTML = projectImagesTemp.map((src, i) => `
...
function uploadProjectImage(input) {
  ...
  if (!file) return;
  const reader = new FileReader();
  reader.onload = e => { projectImagesTemp.push(e.target.result); renderProjectImgList(); };
  reader.readAsDataURL(file);
}
...
function saveProject(id) {
  ...
  if (id) { const idx=projects.findIndex(x=>x.id===id); if(idx>-1) projects[idx]=item; }
  else projects.push(item);
  setData('projects', projects);
  closeDashModal(); renderProjectsList();
}
...
function renderCareersList() {
  ...
  if (!list) return;
  if (!careers.length) { list.innerHTML = '<div class="empty-state" style="grid-column:1/-1"><i class="fas fa-briefcase"></i>No job postings yet</div>'; return; }
...
function toggleCareer(id) {
  ...
  if (c) { c.active = !c.active; setData('careers', careers); renderCareersList(); }
}
...
function updateReqDisplay() {
  ...
  if (el) el.innerHTML = reqTags.map((r,i)=>`<span class="req-tag">${r}<button onclick="removeReqTag(${i})">×</button></span>`).join('');
}
...
function saveCareer(id) {
  ...
  if (id) { const idx=careers.findIndex(x=>x.id===id); if(idx>-1) careers[idx]=item; }
  else careers.push(item);
  setData('careers', careers);
  closeDashModal(); renderCareersList();
}
...
function renderPartnersList() {
  ...
  list.innerHTML = partners.map(p => `
    ...
    </div>`).join('') || '<div class="empty-state"><i class="fas fa-handshake"></i>No partners yet</div>';
}
...
function savePartner(id) {
  const partners = getData('partners') || [];
  const item = { id: id||Date.now(), name: document.getElementById('m-pname').value, logo: document.getElementById('m-plogo').value };
  if (id) { const idx=partners.findIndex(x=>x.id===id); if(idx>-1) partners[idx]=item; } else partners.push(item);
  setData('partners', partners); closeDashModal(); renderPartnersList();
}

function deleteItemFrom(key, id) {
  if (!confirm('Delete this item?')) return;
  const arr = getData(key) || [];
  setData(key, arr.filter(x=>x.id!==id));
...
function renderTestimonialsList() {
  ...
  list.innerHTML = testimonials.map(t => `
    ...
    </div>`).join('') || '<div class="empty-state"><i class="fas fa-star"></i>No testimonials yet</div>';
}
...
function saveTestimonial(id) {
  const testimonials = getData('testimonials') || [];
  const item = { id: id||Date.now(), author: document.getElementById('m-author').value, company: document.getElementById('m-company').value, text_en: document.getElementById('m-text-en').value, text_ar: document.getElementById('m-text-ar').value, stars: parseInt(document.getElementById('m-stars').value)||5 };
  if (id) { const idx=testimonials.findIndex(x=>x.id===id); if(idx>-1) testimonials[idx]=item; } else testimonials.push(item);
  setData('testimonials', testimonials); closeDashModal(); renderTestimonialsList();
}
...
function renderMessagesList() {
  ...
  if (!list) return;
  if (!messages.length) { list.innerHTML = '<div class="empty-state"><i class="fas fa-inbox"></i><p>No messages received yet</p></div>'; return; }
...
function markMsgRead(i) {
  ...
  renderMessagesList(); renderOverview();
}
...
function renderApplicationsList() {
  ...
  if (!list) return;
  if (!apps.length) { list.innerHTML = '<div class="empty-state"><i class="fas fa-user-tie"></i><p>No applications received yet</p></div>'; return; }
  list.innerHTML = apps.reverse().map(a => `
    <div class="msg-row">
...
function renderTeamList() {
  ...
  if (!team.length) { list.innerHTML = superAdmin + '<div class="empty-state" style="margin-top:20px"><i class="fas fa-users"></i><p>No sub-admins yet</p></div>'; return; }
  list.innerHTML = superAdmin + team.map((m, i) => `
    <div class="team-card">
...
function saveTeamMember() {
  ...
  const team = getTeam();
  if (team.find(x=>x.username===u)) return alert('Username already exists');
  team.push({ username: u, password: p, role: r || 'Sub-Admin' });
  setTeam(team);
  closeDashModal(); renderTeamList();
}
...
function renderOurGroupList() {
  ...
  if (!list) return;
  if (!companies.length) {
    list.innerHTML = '<div class="empty-state" style="grid-column:1/-1"><i class="fas fa-city"></i>No companies yet</div>';
    return;
  }
...
function previewUpload(input, targetId) {
  ...
  if (!file) return;
  const reader = new FileReader();
  reader.onload = e => { const el = document.getElementById(targetId); if(el) el.value = e.target.result; };
  reader.readAsDataURL(file);
}
